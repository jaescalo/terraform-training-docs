...
Here, we are looking for the `akamai_group` named "My Group", which can be found in the given contract. This is useful as we may wish to use the group ID but only know its name. Referencing data sources is similar to referencing resources, except data source IDs always start with `data`, whereas resources just start with the resource type.
...
Here we cross-reference the id attribute of the group data source, in order to use it in our `akamai_property` resource.
