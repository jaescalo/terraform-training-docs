...
1. Create a variables.tf file which declares at least variables. Use more than 1 data type (e.g. `string` and `number`). Give 1 (and only 1) of the variables a default value.
2. Create a 2nd .tf file which declares a resource which references all 3 variables.
3. Run `terraform init` and `terraform plan`, providing values for the 2 non-default variables using the `-var` syntax
4. Create a `terraform.tfvars` file and use it to set values for all 3 variables, changing the value of the default value you set in step 1. Re-run `terraform plan` using this file instead.
